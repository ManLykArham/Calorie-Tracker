...
function storeVariablesFood($foodName, $foodAmount, $caloriesGained, $DATE)
{
    ...
    $sql = "INSERT INTO foodlog (userID, mealName, amountGrams, caloriesGained, logDate, logTime	) VALUES (?, ?, ?, ?, ?, ?)";
    $stmt = mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sql)) {
        header("Location: ../PAGES/foodLogWebPage.php?error=sqlerror'insert'");
        exit();
    } else {
        mysqli_stmt_bind_param($stmt, "isiiss", $userID, $foodName, $foodAmount, $caloriesGained, $date, $time);
